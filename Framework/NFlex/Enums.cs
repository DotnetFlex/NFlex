...
namespace NFlex {
    ...
    public static class Enums {
        ...
        public static T GetInstance<T>( object member ) {
            string value = member.To("");
            if( string.IsNullOrWhiteSpace( value ) )
                throw new ArgumentNullException("member");
            return (T)System.Enum.Parse( Common.GetType<T>(), value, true );
        }
        ...
        /// <param name="member">成员名或值,
        /// 范例:Enum1枚举有成员A=0,则传入"A"或"0"获取 Enum1.A</param>
        public static T GetInstance<T>(object member,T defaultValue)
        {
            string value = member.To("");
            if (string.IsNullOrWhiteSpace(value))
                return defaultValue;
            var result= System.Enum.Parse(Common.GetType<T>(), value, true);
            if (result == null)
                return defaultValue;
            else
                return (T)result;
        ...
        public static string GetName<T>( object member ) {
            return GetName( Common.GetType<T>(), member );
        }
        ...
        public static int GetValue<T>( object member ) {
            return GetValue(Common.GetType<T>(), member );
        }
        ...
        public static List<Item> GetItems<T>() {
            Type enumType = Common.GetType<T>();
        ...
        {
            object attribute = field.GetCustomAttributes(typeof(SortAttribute), true).FirstOrDefault();
            if (attribute == null)
                return -1;
            return ((SortAttribute)attribute).SortId;
        }
